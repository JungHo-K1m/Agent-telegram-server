...
def add_persona(name: str, system_prompt: str) -> str:
    return supabase_service.add_persona(name, system_prompt)
...
def save_mapping(agent_id: str, chat_id: int, role: str, persona_id: str, delay: int):
    return supabase_service.save_mapping(agent_id, chat_id, role, persona_id, delay)

def get_mapping(agent_id: str, chat_id: int):
    return supabase_service.get_mapping(agent_id, chat_id)

def delete_mapping(agent_id: str, chat_id: int):
    return supabase_service.delete_mapping(agent_id, chat_id)

def delete_agent_mappings(agent_id: str):
    return supabase_service.delete_agent_mappings(agent_id)
